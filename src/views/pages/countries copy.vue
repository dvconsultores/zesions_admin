...
<v-row>
  ...
  >
          ...
          <v-card-text>
            <v-row class="ma-0 pb-5 px-2">
              ...
              <v-row>
                ...
                <!-- Servicios FIAT por País-->
                <!--v-col cols="6" class="mb-6">
                  <v-card>
                    <v-card-title>
                      Servicios FIAT por País
                      <v-spacer />
                      <v-btn icon @click="dialogDefix = true">
                        <v-icon dark color="primary" v-if="usersAdmin.escribir">
                          {{ icons.mdiCashPlus }}
                        </v-icon>
                      </v-btn>
                    ...
                    <v-card-text>
                      <v-row class="ma-0">
                        <v-col sm="12">
                          <v-data-table :headers="headersDefix" :items="dataDefix" :items-per-page="10" :search="search"
                            class="elevation-1">
              
                            <template #[`item.habilitado`]="{ item }">
                              <v-switch v-model="item.habilitado" @change="actualizarEstatusDefix(item)" />
                            </template>
              
                            <template #[`item.acciones`]="{ item }">
                              <v-btn icon dark color="primary" @click="openEditarDefix(item)">
                                <v-icon>{{ icons.mdiPencil }}</v-icon>
                              </v-btn>
                              <v-btn icon dark color="#E53935" @click="openDeleteDefix(item)">
                                <v-icon>{{ icons.mdiDelete }}</v-icon>
                              </v-btn>
                            </template>
                          </v-data-table>
    ...
    <!-- DIALOG CREAR  servicio defix PAIS-->
    <v-dialog
      v-model="dialogDefix"
      persistent
      max-width="600px"
      ...
      <v-card>
        ...
        >
          <v-btn
            ...
            icon
            class="white--text"
            @click="dialogDefix = false"
          >
            <v-icon>{{ icons.mdiClose }}</v-icon>
          </v-btn>
          <v-toolbar-title class="white--text">Crear nuevo Servicio FIAT por País</v-toolbar-title>
        </v-toolbar>
        <v-container>
          <v-card-text>
            <v-row class="ma-0 pb-5 px-2">
              ...
              <v-col class="col-6">
                ...
                v-model="datos"
                  outlined
                  label="Datos Servicio FIAT"
                  placeholder="Datos Descripcion del Servicio"
                  hide-details
                  class="mb-3"
                  dense
                ></v-textarea>
              ...
              <v-btn
                ...
                color="primary"
                class="mt-6"
                @click="dialogDefix = false, createCountryDefix()"
              >
                Guardar
              </v-btn>
            </v-row>
    ...
    <!-- FIN DIALOG CREAR servicio defix PAIS-->
    ...
    <!-- DIALOG EDITAR servicio defix PAIS-->
    <v-dialog
      v-model="dialogeditDefix"
      persistent
      max-width="600px"
    >
      <v-card>
        ...
        >
          <v-btn
            ...
            icon
            class="white--text"
            @click="dialogeditDefix = false"
          >
            <v-icon>{{ icons.mdiClose }}</v-icon>
          </v-btn>
          <v-toolbar-title class="white--text">Edición de Servicios FIAT por País</v-toolbar-title>
        </v-toolbar>
        <v-container>
          <v-card-text>
            <v-row class="ma-0 pb-5 px-2">
              <v-col class="col-6">
                <v-text-field
                  v-model="registroDefix.nombre"
                  outlined
                  label="Servicio FIAT"
                  placeholder="Documento x Servicio"
                  hide-details
                  class="mb-3"
                  dense
                />
              ...
              <v-col class="col-6">
                <v-textarea
                  v-model="registroDefix.datos"
                  outlined
                  label="Datos Servicio FIAT"
                  placeholder="Datos Documento x Servicio"
                  hide-details
                  class="mb-3"
                  dense
                ></v-textarea>
              ...
              <v-col class="col-6 mb-0">
                <v-switch
                  v-model="registroDefix.habilitado"
                  label="El Servicio esta habilitado?"
                  outlined
                  dense
                />
              </v-col> 
              ...
              <v-btn
                ...
                color="primary"
                class="mt-6"
                @click="dialogeditDefix = false, guardarEdicionDefix()"
              >
                Guardar
              </v-btn>
    ...
    <!-- FIN DIALOG EDITAR servicio defix PAIS-->
            ...
            <v-row class="ma-0 pb-5 px-2">
              ...
              <v-col class="col-6">
                <v-text-field
                  ...
                  outlined
                  label="Id"
                  placeholder="Nro. identidicación"
                  hide-details
                  class="mb-3"
                  dense
                />
    ...
    <!-- DIALOG DELETE servicios defix X PAIS -->
    <v-dialog
      v-model="dialogSureDefix"
      max-width="290"
      transition="dialog-top-transition"
    >
      <v-card>
    ...
    <!-- FIN DIALOG DELETE servicios defix X PAIS --> 
...
export default {
  data() {
    return {
      ...
      headersBanco: [
        { text: 'Nombre', value: 'nombre'},
        { text: 'Código (4 Digitos)', value: 'codigo'},
        { text: 'Estatus' , value: 'habilitado'},
        { text: 'Acciones', value: 'acciones'},
      ],dataBanco:[],
  ...
  methods: {
    ...
    fecthDataDefix (id) {
      this.axios.get('/paisesdefix/?pais='+id).then(response => {
          this.dataDefix = response.data
          //this.fecthData()
        }).catch(err => {
          console.log(err)
        })
    ...
    fecthDataCuenta (id) {
//      this.axios.get('/kyccuentadefix/?pais='+id).then(response => { '/get-tpagos/'+id+''
    ...
    createCountry () {
      console.log(this.registro.imagen)
    ...
    createCountryDefix () {
      ...
      console.log(data)
      this.axios.post('/paisesdefix/', data).then((res) => {
        console.log(res.data)
        this.snackbar = true
        this.color = 'primary'
        this.text = 'Se guardo correctamente'
        this.nombre = ''
        this.datos = ''
        this.fecthDataDefix(this.registro.id)
      }).catch((err) => {
        console.log(err)
        this.snackbar = true
        this.color = '#E53935'
    ...
    actualizarEstatusDefix(item){  
        this.axios.patch('/paisesdefix/'+ item.id + '/', {'habilitado':item.habilitado}).then((res) => {
        console.log(res.data)
        this.snackbar = true
        this.color = 'primary'
        this.text = 'Se actualizo el estatus correctamente'
    ...
    openEditarDefix(item){
      console.log(item)
      this.dialogeditDefix = true
      this.registroDefix.id = item.id
      this.registroDefix.nombre = item.nombre
      this.registroDefix.datos = item.datos
      this.registroDefix.habilitado = item.habilitado
    },
    ...
    guardarEdicionDefix() {
        const formData = new FormData()
        formData.append('nombre', this.registroDefix.nombre)
        formData.append('datos', this.registroDefix.datos)
        formData.append('habilitado', this.registroDefix.habilitado)
        this.axios.patch('/paisesdefix/'+ this.registroDefix.id + '/', formData).then((res) => {
        this.snackbar = true
        this.color = 'primary'
        this.text = 'Se actualizo correctamente'
        this.fecthDataDefix(this.registro.id)
    ...
    openDeleteDefix (item) {
      this.dataDeleteDefix = item
      this.dialogSureDefix = true
    },
    ...
    eliminarDefix () {
      this.dialogSureDefix = false
      this.axios.delete('/paisesdefix/' + this.dataDeleteDefix.id + '/').then(res => {
        console.log(res)
        this.snackbar = true
        this.color = 'primary'
        this.text = 'Se elimino correctamente'
